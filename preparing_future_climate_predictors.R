...
### Equivalent to "preparing_predictors.R", but only for future climate
###
### Requires: aoi, future climate tifs (output of processing_climate.R)
### Outputs: Future_Climate_RCP85_2050s.csv, which has 25th and 75th projections by pid
### 6/23/20
###
...
# Note that choosing "band = 2" pulls in the 25th percentile data. Band = 3 corresponds to 75th percentil
...
climate_points <- aoi_centers %>%
  mutate(precip25 = raster::extract(precip25, aoi_centers),
...
# write it out
write_csv(climate_pids, "mar_tourism/Data/Future_Climate_RCP85_2050s.csv")
