##
## This Version of create_country_IPMs is appropriate to be used for any scenario, but was developed
## on 4/15/21 and only USED to create the fire risk ipms for MX. (The other scenarios are too far along
## to be worth updating).
## The difference is that it only creates IPM raster tourism values for hexes that fall entirely within
## the ROOT AOI. This is a necessary patch because the "conversion" rasters accidentally give huge weight
## to hex slivers that fall inside the Root AOI, assuming that those slivers are the total area across
## which tourists should be spread, when, in fact, the actual hex extends beyond the root aoi.
...
for(country in countries){
  ...
  # clip to only include hexes which are completely inside the root AOI (to avoid edge effects), added 4/15/21
  inside <- st_covered_by(country_tran, country_sf)
  country_inside <- country_tran[lengths(inside) > 0, ]
  ggplot(country_inside) + geom_sf()
  
  
  diff_rast <- fasterize(country_inside, country_rast, field = "diff_vis", background = 0)
  diff_rast
