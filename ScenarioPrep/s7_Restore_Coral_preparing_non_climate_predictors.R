####
#### Restore Coral Preparing Predictors
#### Forked from `preparing_non_climate_predictors.R` on 12/22/20
###
...
coral <- read_sf("ROOT/07_rest_corl/T_AOI_07_rest_corl_coral.geojson")
...
# do the same for coral (Note that for the restore coral scenario these columns are name "count" instead of "sum)
coral
coral_pid <- coral %>% 
  st_set_geometry(NULL) %>%
  mutate(coral_prop = if_else(is.na(c0_count), 0, c0_count / (area*multiplier)),
         coral_prop25 = if_else(is.na(c1_count), 0, c1_count / (area*multiplier)),
         coral_prop75 = if_else(is.na(c2_count), 0, c2_count / (area*multiplier))) %>%
...
## Write it out
write_sf(preds_mangrove, paste0("mar_tourism/Data/Scenarios/s7_rest_corl_NonClimatePredictors_", dddd, ".geojson"), delete_dsn = TRUE)
write_csv(preds_mangrove %>% st_set_geometry(NULL), paste0("mar_tourism/Data/Scenarios/s7_rest_corl_NonClimatePredictors_", dddd, ".csv"))
