####
#### RESTORE MANGROVEs Predictor Prep
###
...
# Coral - % coverage (poly)
# Mangrove - % coverage (poly)
# Wildlife - p/a (poly)
# Forest - % coverage (handled earlier)
...
mangrove2_geom <- mangrove2 %>% dplyr::select(geometry)

mangrove <- rbind(mangrove1_geom, mangrove2_geom) #sloppy, but functional
...
## Write it out
write_sf(preds_mangrove, paste0("mar_tourism/Data/Scenarios/s4_restore_mangroves_NonClimatePredictors_", dddd, ".geojson"), delete_dsn = TRUE)
write_csv(preds_mangrove %>% st_set_geometry(NULL), paste0("mar_tourism/Data/Scenarios/s4_restore_mangroves_NonClimatePredictors_", dddd, ".csv"))
